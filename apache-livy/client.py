...
class HttpClient(object):
    ...
    def __init__(self, url, load_defaults=True, conf_dict=None):
        ...
        if match:
            ...
            session_response = self._create_new_session(session_conf_dict)
            if session_response.status_code != 200:
                raise Exception("Failed creating a session. Http status: %d" % session_response.status_code)
            self._session_id = session_response.json()['id']
...
class _LivyConnection(object):
    ...
    def _spnego_auth(self):
        if self._spnego_enabled:
            ...
            from requests.auth import HTTPBasicAuth
            client = HTTPBasicAuth('admin', '!Qq12345678')
            return client
